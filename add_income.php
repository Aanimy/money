...
try {
    ...
    // 获取收入分类
    $categories = $db->query("SELECT * FROM income_categories ORDER BY sort_order")->fetchAll();
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        try {
            // 添加收入记录
            $stmt = $db->prepare("
                INSERT INTO incomes (user_id, category_id, account_id, amount, income_date, description)
                VALUES (?, ?, ?, ?, ?, ?)
            ");
            $stmt->execute([
                $user_id,
            ...
            // 记录操作日志
            Logger::getInstance()->logUserAction($user_id, 'add_income', "添加收入记录：{$amount}元");
